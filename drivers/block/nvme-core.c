			  ...
			  unsigned int i)
{
	...
	struct nvme_queue *nvmeq = raw_nvmeq(dev, i + 1);
	if (!nvmeq)
		return -ENODEV;
	WARN_ON(nvmeq->hctx);
	nvmeq->hctx = hctx;
				  ...
				  unsigned int tag)
{
	...
	struct request *req;
WARN(!hctx, "hctx for queue %d is null\n", nvmeq->qid);
	req = blk_mq_tag_to_rq(hctx->tags, tag);
WARN(!req, "blk_mq_tag_to_rq for tag %d queue %d is null\n", tag, nvmeq->qid);
...
{
	/* TODO: figure out if kmalloc_node() is better */
...
{
	struct request *req = blk_mq_alloc_request(ns->queue, WRITE,
								GFP_KERNEL);
							  ...
							  struct request *req)
{
	...
	if (req->cmd_flags & REQ_DISCARD) {
		...
		dma_dir = rq_data_dir(req) ? DMA_TO_DEVICE : DMA_FROM_DEVICE;
	        ...
	        result = nvme_map_rq(nvmeq, iod, req, dma_dir, psegs);
		if (result <= 0)
                	goto finish_cmd;
								...
								u32 *result)
{
	...
	req = blk_mq_alloc_request(dev->admin_rq, WRITE, GFP_KERNEL);
	if (!req)
		return -ENOMEM;
	res = nvme_submit_sync_cmd(req, cmd, result, ADMIN_TIMEOUT);
	blk_put_request(req);
	return res;
}
...
static void nvme_free_queue(struct rcu_head *r)
{
	...
	/* TODO: force a flush/cancel of outstanding & queued ios */
			...
			struct nvme_id_ns *id, struct nvme_lba_range_type *rt)
{
	...
	if (list_empty(&dev->namespaces)) {
		...
		struct nvme_ns *first_ns =
				list_first_entry_or_null(&dev->namespaces,
							struct nvme_ns, list);
		if (first_ns)
			ns->queue = first_ns->queue;
	}
	...
	if (dev->max_hw_sectors)
		...
		blk_queue_max_hw_sectors(ns->queue, 4096);
...
static void nvme_assign_io_queues(struct nvme_dev *dev)
{
	...
	for (i = 1; i <= queues; i++) {
		...
		cpumask_t mask;
WARN(!nvmeq, "lock_nvmeq NULL for q %d\n", i);
