/*
 * kvblade - ATA over Ethernet
 *
 * Copyright (C) 2006 Coraid, Inc.
 * Copyright (C) 2015 Matias Bjorling <m@bjorling> sysfs, cleanup, rebase, bugs.
 */
...
#define MAXSECTORS(mtu) (((mtu) - sizeof(struct aoe_hdr) \
					- sizeof(struct aoe_atahdr)) / 512)
...
static ssize_t kvblade_add(u32 major, u32 minor, char *ifname, char *path)
{
	...
	if (!nd) {
		pr_err("kvblade: add failed: interface %s not found.\n",
							ifname);
		return -ENOENT;
	}
	dev_put(nd);
	...
	if (IS_ERR_OR_NULL(bd)) {
		pr_err("kvblade: add failed: can't open block device %s: %ld\n",
							path, PTR_ERR(bd));
		return -ENOENT;
	}

	if (!get_capacity(bd->bd_disk)) {
		pr_err("kvblade: add failed: zero sized block device.\n");
		ret = -ENOENT;
		goto err;
	}
	...
	for (td = devlist; td; td = td->next)
		if (td->major == major &&
			...
			pr_err("kvblade: add failed: device %d.%d already exists on %s.\n",
							major, minor, ifname);
	...
	d = kzalloc(sizeof(struct aoedev), GFP_KERNEL);
	if (!d) {
		ret = -ENOMEM;
		goto err;
	}
	...
	kobject_init_and_add(&d->kobj, &kvblade_ktype, &kvblade_kobj,
					"%d.%d@%s", major, minor, ifname);

	d->next = devlist;
	devlist = d;
	spin_unlock(&lock);

	pr_debug("kv_blade: added %s as %d.%d@%s: %llu sectors.\n",
...
static ssize_t kvblade_del(u32 major, u32 minor, char *ifname)
{
	...
	if (!d) {
		pr_err("kv_blade: del failed: device %d.%d@%s not found.\n",
							major, minor, ifname);
		ret = -ENOENT;
		goto err;
	} else if (atomic_read(&d->busy)) {
		pr_err("kvblade: del failed: device %d.%d@%s is busy.\n",
							major, minor, ifname);
		ret = -EBUSY;
		goto err;
	}
...
static ssize_t store_add(struct aoedev *dev, const char *page, size_t len)
{
	...
	if (kvblade_sysfs_args(p, argv, nelem(argv)) != 4) {
		pr_err("kvblade: bad arg count for add\n");
		error = -EINVAL;
	} else
...
static struct kvblade_sysfs_entry kvblade_sysfs_add =
					__ATTR(add, 0644, NULL, store_add);
...
static ssize_t store_del(struct aoedev *dev, const char *page, size_t len)
{
	...
	if (kvblade_sysfs_args(p, argv, nelem(argv)) != 3) {
		pr_err("kvblade: bad arg count for del\n");
		error = -EINVAL;
	} else
		error = kvblade_del(simple_strtoul(argv[0], NULL, 0),
				    simple_strtoul(argv[1], NULL, 0), argv[2]);

	kfree(p);
	return error ? error : len;
}

static struct kvblade_sysfs_entry kvblade_sysfs_del =
				__ATTR(del, 0644, NULL, store_del);
...
static struct kvblade_sysfs_entry kvblade_sysfs_scnt =
				__ATTR(scst, 0644, show_scnt, NULL);
...
static struct kvblade_sysfs_entry kvblade_sysfs_bdev =
				__ATTR(bdev, 0644, show_bdev, NULL);
...
static struct kvblade_sysfs_entry kvblade_sysfs_bpath =
				__ATTR(bpath, 0644, show_bpath, NULL);
...
static struct kvblade_sysfs_entry kvblade_sysfs_model =
				__ATTR(model, 0644, show_model, store_model);
...
static struct kvblade_sysfs_entry kvblade_sysfs_sn =
				__ATTR(sn, 0644, show_sn, store_sn);
...
static ssize_t kvblade_attr_show(struct kobject *kobj, struct attribute *attr,
								char *page)
{
...
static void ata_io_complete(struct bio *bio, int error)
{
	...
	len = sizeof(struct aoe_hdr) + sizeof(struct aoe_atahdr);
	if (bio_flagged(bio, BIO_UPTODATE)) {
		...
		pr_err("kvblade: I/O error %d on %s\n", error, d->kobj.name);
...
static struct sk_buff *ata(struct aoedev *d, struct sk_buff *skb)
{
	...
	do {
		...
		if ((lba + ata->scnt) > d->scnt) {
			pr_err("kvblade: sector I/O is out of range: %llu (%d), max %llu\n",
		...
		if (bio == NULL) {
			pr_err("kv_blade: can't alloc bio\n");
			goto drop;
		}
		rq->bio = bio;
		rq->d = d;
		...
		if (bio_add_page(bio, page, bcnt, offset) < bcnt) {
			pr_err("kvblade: can't bio_add_page for %d sectors\n",
								ata->scnt);
			bio_put(bio);
			goto drop;
		}
		...
		return NULL;
	default:
		pr_err("kvblade: unknown ATA command 0x%02X\n", ata->cmdstat);
		ata->cmdstat = ATA_ERR;
...
static struct sk_buff *cfg(struct aoedev *d, struct sk_buff *skb)
{
	...
	case AOECCMD_READ:
		cfg->cslen = cpu_to_be16(d->nconfig);
		memcpy(cfg->data, d->config, d->nconfig);
		len += sizeof(struct aoe_cfghdr) + d->nconfig;
		break;
	case AOECCMD_SET:
		if (d->nconfig)
			if (d->nconfig != cslen ||
			    memcmp(cfg->data, d->config, cslen) != 0) {
				aoe->verfl |= AOEFL_ERR;
				aoe->err = AOEERR_CFG;
				break;
			}
	...
	case AOECCMD_FSET:
		...
		len += sizeof(struct aoe_cfghdr) + cslen;
		break;
	default:
		aoe->verfl |= AOEFL_ERR;
		aoe->err = AOEERR_ARG;
...
static int rcv(struct sk_buff *skb, struct net_device *ndev,
...
static int kthread(void *errorparameternameomitted)
{
	...
	do {
		...
		do {
			iskb = skb_dequeue(&skb_inq);
			if (iskb)
				ktrcv(iskb);

			oskb = skb_dequeue(&skb_outq);
			if (oskb)
				dev_queue_xmit(oskb);
		} while (iskb || oskb);
...
static int __init kvblade_module_init(void)
{
	...
	kobject_init_and_add(&kvblade_kobj, &kvblade_ktype_ops, NULL,
								"kvblade");
